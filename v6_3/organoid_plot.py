...
Görev: Veriyi görsel olarak inceleme imkanı sağlamak.
Yorum yok — etiketler nötr ("CV", "ISI", "Spike rate"),
              değerlendirme yok ("normal", "yüksek" yazılmaz).

İki backend destekli:
  - matplotlib (default): statik PNG, hızlı, yayına uygun
  - plotly (--plotly): interaktif HTML, zoom/hover destekli
"""
...
def quick_look_plotly(spike_zaman, sure_sn, cikti_yolu, baslik_ek=''):
    """
    Plotly tabanlı interaktif HTML çıktısı.
    ...
    try:
        ...
        raise ImportError('plotly kurulu degil. pip install plotly')
    ...
    rates, zamanlar = [], []
    t = 0.0
    while t + pencere <= sure_sn:
        n = int(np.sum((spike_zaman >= t) & (spike_zaman < t + pencere)))
        rates.append(n / pencere)
        zamanlar.append(t + pencere/2)
        t += pencere
    fig.add_trace(go.Scatter(
        x=zamanlar, y=rates,
        mode='lines',
...
def quick_look(spike_zaman, sure_sn, cikti_yolu,
    ...
    Üç panelli quick-look grafiği:
      ...
      3. Spike rate zaman serisi

    backend : 'matplotlib' (default) veya 'plotly'
              plotly seçildiyse cikti_yolu .html olmalı.
    """
    if backend == 'plotly':
    ...
    if len(spike_zaman) > 0:
        # Performans için max 10000 spike göster
        n_max = 10000
    ...
    ax2.set_xlabel('ISI (ms, log scale)', fontsize=9)
    ax2.set_ylabel('Sayi', fontsize=9)
    for s in ['top', 'right']:
        ax2.spines[s].set_visible(False)

    # Panel 3: Spike rate zaman serisi
    ...
    if rates:
        ...
        ax3.fill_between(zamanlar, 0, rates, color='steelblue',
                          alpha=0.25)
    ax3.set_title('Spike rate (1 sn pencere)', fontsize=10)
...
def cv_plot(cv_serisi, cikti_yolu, baslik_ek=''):
    """
    Pencere bazlı CV zaman serisi grafiği.
    Yorum bantları yok, sadece veri ve y=1 referansı (Poisson).
    """
    if not cv_serisi:
        return None
    ...
    cv_vals = [x[1] for x in cv_serisi]
    ax.plot(zamanlar, cv_vals, 'o-', color='darkblue',
             markersize=4, linewidth=1.2, alpha=0.85)
    ax.axhline(y=1.0, color='gray', linestyle='--',
                linewidth=0.7, alpha=0.5, label='CV = 1 (Poisson ref)')
