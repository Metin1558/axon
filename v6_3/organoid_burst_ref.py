...
def bakkum_burst_tespit(spike_listesi, sure_sn,
    ...
    [DÜZELTME — bkz. proje notları] Varsayılan esik_carpan önceden
    2.5 idi, ama bu docstring'in kendisi (aşağıda) Bakkum 2013'ün
    5 kullandığını söylüyordu — kod ile referans arasında çelişki
    vardı. Bu modülün TEK amacı Bakkum'un yöntemiyle karşılaştırma
    olduğu için, Bakkum'un kendi eşiğini kullanmayan bir "Bakkum
    karşılaştırması" kendi amacını geçersiz kılar. Varsayılan artık
    gerçekten 5 — referansla birebir.
    ...
    esik_carpan: baseline'ın kaç katı eşik (Bakkum 2013'te 5 kullanılmış — varsayılan buna eşit)
...
def burst_karsilastir(spike_listesi, sure_sn, cikti_yolu=None):
    ...
    bakkum_bin_ms = 50  # bakkum_burst_tespit'in varsayılanıyla birebir - grafik ekseni için de kullanılacak
    bakkum_burstler, pop_rate = bakkum_burst_tespit(
        spike_listesi, sure_sn, bin_ms=bakkum_bin_ms)
    ...
    if cikti_yolu:
        _grafik_uret(mad_burstler, bakkum_burstler, pop_rate,
                     sure_sn, iou_sonuc, cikti_yolu, bin_ms=bakkum_bin_ms)

    return sonuc


def _grafik_uret(mad_burstler, bakkum_burstler, pop_rate,
                  sure_sn, iou_sonuc, cikti_yolu, bin_ms=50):
    """
    [DÜZELTME — bkz. proje notları] Zaman ekseni önceden hep
    "10ms bin" varsayarak (`* 10 / 1000.0`) çiziliyordu, ama pop_rate
    burada gerçekte `bakkum_burst_tespit`'in kullandığı bin_ms (varsayılan
    50ms) ile hesaplanıyor. Bu, çizilen popülasyon-hızı eğrisinin gerçek
    süreye göre 5 kat sıkıştırılmış görünmesine, dolayısıyla burst
    kutucuklarının (gerçek saniyelerle çizilen) eğriyle hizasız
    durmasına yol açıyordu. Sayısal çıktılar (IoU, F1, burst sayıları)
    bundan etkilenmiyordu — sadece PNG görseli yanlıştı. bin_ms artık
    parametre olarak geçiriliyor, hardcode edilmiş "10" kaldırıldı.
    """
    import matplotlib
    matplotlib.use('Agg')
    ...
    t = np.arange(len(pop_rate)) * bin_ms / 1000.0  # gerçek bin_ms -> sn
