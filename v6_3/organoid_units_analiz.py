...
def unit_analiz(unit_id, spike_zaman, sure_sn, refractory_ms=1.0,
    ...
    # Refractory violation (Wilson CI)
    refr = oqc.refractory_ihlget_orani(spike_zaman, refractory_ms=refractory_ms)
    
    # Negatif ISI (sirali check)
...
def cross_unit_senkron(spike_listesi, sure_sn, dt=0.01, max_unit=20):
    ...
    print(f'  STTC matrisi: en active {len(secilen_treni)} unit (total {n_unit})...')
    
    sonuc = omet.coklu_channel_senkron(secilen_treni, sure_sn, dt=dt)
...
def network_burst_tespit(spike_listesi, sure_sn, bin_ms=50,
    ...
    for sp in spike_listesi:
        ...
        # Active unit sayaci (bu unit'in DOKUNDUGU bin'lere +1)
        aktif_unit_per_bin[unique_bins] += 1
    
    # Adaptif esik: median + carpan * MAD
    # Sparse datade (cogu bin 0) median/MAD calismaz - sifir olmayanlari kullan
    nonzero = pop_rate[pop_rate > 0]
    if len(nonzero) > 10:
        # Yeteri kadar data varsa MAD ile
    ...
    # Esik en az min_active_unit (mantigen burst en az min_active_unit'i icermeli)
...
def grafik_uret(spike_listesi, unit_sonuclari, pop_metrik, burst_sonuc,
    ...
    # Panel 1: Raster (en active 30 unit, yoksa hepsi)
...
def main():
    ...
    if not os.path.exists(args.dosya):
        print(f'ERROR: File yok: {args.dosya}')
        return 1
    ...
    # 1. Data oku
    # ========================================
    print('[1/5] NWB filesi reading...')
    
    units = oio.sorted_units_read(args.dosya)
    if not units:
        print('  WARNING: NWB filesinda sorted units tablosu bulunamadi.')
        print()
        print('  Bu file ham elektriksel sinyal iceriyor olabilir.')
        print('  Ham sinyal analysisi farkli bir islem akisi gerektirir:')
        print('  bandpass filtre + spike detection + spike sorting.')
        print()
        print('  Ham datayi islemek ister misiniz?')
        print('  1 : Ham sinyal modu (basit threshold detection)')
        print('  2 : SpikeInterface ile gercek spike sorting')
        print('  0 : Hayir — cik')
        print()
        
        try:
            secim = input('  Seciminiz (0/1/2): ').strip()
        except (EOFError, KeyboardInterrupt):
            secim = '0'

        if secim == '2':
            print()
            print('  SpikeInterface modu baslatiliyor...')
            try:
                import si_sorting
                spike_dict = si_sorting.interaktif_sort(args.dosya)
                if not spike_dict:
                    print('  Sorting iptal edildi veya basarisiz.')
                    return 1
                import numpy as np
                # Yeni format: result dict
                if isinstance(spike_dict, dict) and 'spike_dict' in spike_dict:
                    result = spike_dict
                    spike_dict = result['spike_dict']
                    quality_dict = result.get('quality', {})
                    print(f"  {result['n_units']} unit sorted "
                          f"({result.get('n_kaliteli', '?')} kaliteli)")
                else:
                    quality_dict = {}
                    print(f"  {len(spike_dict)} unit sorted.")

                units = []
                for uid, times in spike_dict.items():
                    q = quality_dict.get(uid, {})
                    units.append({
                        'unit_id': uid,
                        'spike_zaman': times,
                        'spike_sayisi': len(times),
                        'snr': q.get('snr'),
                        'isi_violation_ratio': q.get('isi_violation_ratio'),
                        'kaliteli': q.get('kaliteli', True),
                    })
                # units ile devam et — asagidaki analiz blogu kullanir
            except ImportError:
                print('  HATA: si_sorting.py bulunamadi.')
                print('  si_sorting.py dosyasini calistirma_klasorune kopyala.')
                return 1
            ...
            try:
                ...
                if tip not in ('ham', 'her_ikisi'):
                    print('  ERROR: Bu fileda ham sinyal de bulunamadi.')
                    print('  File ne sorted units ne de ham sinyal iceriyor.')
                    return 1
                
                print('  Ham sinyal found. Isleniyor...')
                print('  (Bu islem birkac minute surebilir)')
                print()
                
                tum_spike_ham = []
                sure_sn = 0.0
                
                for chunk, bas, bit, sr in oio2.ham_chunk_uret(args.dosya, chunk_sn=10, channel=0, max_sure_sn=180.0):
                    bas_sample = int(bas) if not isinstance(bas, tuple) else int(bas[0])
                    sr_int = int(sr) if not isinstance(sr, tuple) else int(sr[0])
                    sure_sn = bas_sample / sr_int
                    # chunk 1D (tek kanal) veya 2D (cok kanal) olabilir
                    import numpy as np
                    chunk = np.array(chunk)
                    if chunk.ndim == 1:
                        sinyal = chunk
                        spike_idx, _ = osig.spike_tespit_chunk(sinyal, sr_int)
                        spike_zaman = (spike_idx / sr_int) + (bas_sample / sr_int)
                ...
                print(f'  Total spike: {len(tum_spike_ham)}')
                print(f'  Kayit suresi: {sure_sn:.1f} s ({sure_sn/60:.1f} min)')
                print()
                print('  Ham sinyal analysisi tamamlandi.')
                print('  Per-unit analysis icin spike sorting gereklidir.')
                return 0
                
            except Exception as e:
                print(f'  Ham sinyal analysisi errorsi: {e}')
                print('  Sorted units olmadan tam analysis yapilamaz.')
                return 1
        else:
            print('  Exiting.')
            return 1
    
    print(f'  {len(units)} unit found')
    ...
    if not tum_spike:
        print('  ERROR: Hicbir unit\'te spike yok.')
        return 1
    ...
    if not tum_spike_temiz:
        print('  ERROR: Tum spike zamanlari NaN veya gecersiz.')
        return 1
    ...
    print(f'  Active (>={args.min_spike} spike): {aktif_unit_sayi} unit')
    print()
    
    # ========================================
    # 3. Cross-unit synchrony (STTC)
    # ========================================
    print('[3/5] Cross-unit synchrony analysisi (STTC)...')
    sttc_sonuc = cross_unit_senkron(spike_listesi, sure_sn,
                                       max_unit=args.top_units)
    if sttc_sonuc:
        print(f'  Median STTC: {sttc_sonuc["medyan_sttc"]:.3f}')
    ...
    # ========================================
    # 5. Populasyon metrikleri + plot + CSV
    # ========================================
    print('[5/5] Populasyon ozeti, CSV ve plot...')
    ...
    if 'plot_yolu' in dir() or 'grafik_yolu' in locals():
        pass  # plot kaydedildi
    ...
    print('=' * 60)
    print(f'  Kayit suresi    : {sure_sn:.1f} s ({sure_sn/60:.1f} minute)')
    print(f'  Total unit     : {len(units)}')
    print(f'  Active unit      : {aktif_unit_sayi} '
    ...
    print()
    print('  -- Cross-Unit Synchrony (STTC) --')
    if sttc_sonuc:
        print(f'    Median STTC  : {sttc_sonuc["medyan_sttc"]}')
    ...
    if burst_sonuc.get('burst_sayisi', 0) > 0:
        print(f'    Median burst sure : '
              f'{burst_sonuc.get("medyan_burst_sure_sn", 0):.3f} sn')
        print(f'    Median active unit : '
              f'{burst_sonuc.get("medyan_aktif_unit", 0)}')
    print('=' * 60)
    
    sure = time.time() - t0
    print(f'\n  Total sure: {sure:.1f} s')
