...
def analiz_calistir(args):
    ...
    meta = oio.nwb_metadata_read(dosya)
    ...
    if (not args.sort) and tip in ('ham', 'her_ikisi') and spike_zaman is None:
        ...
        if ilk_chunk_f is not None and ilk_spike_idx is not None:
            snr, snr_genlik_uv, _ = osig.snr_compute(
        ...
        units = oio.sorted_units_read(dosya)
        if units:
    ...
    if meta.get('stim_var'):
        stim_bas, stim_bit = oio.stim_channeli_searchlik(dosya)
        if stim_bas is not None:
            stim_es = omet.stim_oncesi_sirasi_sonrasi(
                spike_zaman, sure_sn, stim_bas, stim_bit)

    if meta.get('trial_var'):
        trials = oio.trial_read(dosya)
        stim_trial = omet.trial_stim_kardeleteastir(spike_zaman, trials)

    # 9. QC metrikleri
    qc = oqc.qugetity_metrikleri_topla(
        spike_zaman, sure_sn,
        snr=snr,
    ...
    oout.csv_write(csv_yolu, organoid, kayit, dosya, meta,
    ...
    # 13. Terminal
    oout.terminget_write(organoid, kayit, dosya, meta,
                       isi_metrik, qc, aktiflik, fft_list, welch_list,
                       perm, stim_es, stim_trial)
